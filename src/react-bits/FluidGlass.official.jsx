...
export default function FluidGlass({ mode = 'lens', lensProps = {}, barProps = {}, cubeProps = {}, children, hideScenePlane = false, eventSource }) {
  ...
  return (
    ...
    >
      <ScrollControls damping={0.2} pages={3} distance={0.4}>
        ...
        <Wrapper modeProps={modeProps} hideScenePlane={hideScenePlane} eventSource={eventSource}>
...
const ModeWrapper = memo(function ModeWrapper({
  ...
  useEffect(() => {
    ...
    const target = eventSource || document;
    const onPointerMove = event => {
      const rect = target === document ? { left: 0, top: 0, width: window.innerWidth, height: window.innerHeight } : target.getBoundingClientRect();
      if (!rect.width || !rect.height) return;
      localPointer.current.x = ((event.clientX - rect.left) / rect.width) * 2 - 1;
      localPointer.current.y = -((event.clientY - rect.top) / rect.height) * 2 + 1;
    };
    target.addEventListener('pointermove', onPointerMove, { passive: true });
    return () => target.removeEventListener('pointermove', onPointerMove);
  }, [eventSource, followPointer]);
  ...
  useFrame((state, delta) => {
    ...
    const activePointer = eventSource && followPointer ? localPointer.current : pointer;
  ...
  return (
    <>
      ...
      {!hideScenePlane && <mesh scale={[vp.width, vp.height, 1]}>
        <planeGeometry />
